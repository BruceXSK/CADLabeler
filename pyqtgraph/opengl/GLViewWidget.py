...
class GLViewWidget(QtWidgets.QOpenGLWidget):
    keyPressSig = QtCore.pyqtSignal(object)
    keyReleaseSig = QtCore.pyqtSignal(object)
    mousePressSig = QtCore.pyqtSignal(object)
    mouseReleaseSig = QtCore.pyqtSignal(object)
    mouseMoveSig = QtCore.pyqtSignal(object)
    cameraChangeSig = QtCore.pyqtSignal(object)
    ...
    def mousePressEvent(self, ev):
        self.mousePressSig.emit(ev)
        lpos = ev.position() if hasattr(ev, 'position') else ev.localPos()
        self.mousePos = lpos
        
    def mouseMoveEvent(self, ev):
        self.mouseMoveSig.emit(ev)
        ...
        if ev.buttons() == QtCore.Qt.MouseButton.LeftButton:
            ...
            # print(f'x:{self.cameraPosition()[0]} y:{self.cameraPosition()[1]} z:{self.cameraPosition()[2]} elevation:{self.opts["elevation"]} azimuth:{self.opts["azimuth"]}')
            self.cameraChangeSig.emit(None)
            ...
            if (ev.modifiers() & QtCore.Qt.KeyboardModifier.ControlModifier):
                ...
                self.pan(diff.x(), diff.y(), 0, relative='view')
                # self.pan(diff.x(), diff.y(), 0, relative='global')
            self.cameraChangeSig.emit(None)
            # print(f'x:{self.cameraPosition()[0]} y:{self.cameraPosition()[1]} z:{self.cameraPosition()[2]} elevation:{self.opts["elevation"]} azimuth:{self.opts["azimuth"]}')
        
    def mouseReleaseEvent(self, ev):
        self.mouseReleaseSig.emit(ev)
        pass
